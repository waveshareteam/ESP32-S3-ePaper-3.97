...
unsigned char QMI8658_write_reg(unsigned char reg, unsigned char value)
{
    ...
    return (i2c_write_reg(QMI8658_slave_addr, reg, &value, 1) == ESP_OK) ? 1 : 0;
}

unsigned char QMI8658_write_regs(unsigned char reg, unsigned char *value, unsigned char len)
{
    // Write to multiple registers consecutively
    return (i2c_write_reg(QMI8658_slave_addr, reg, value, len) == ESP_OK) ? 1 : 0;
}

unsigned char QMI8658_read_reg(unsigned char reg, unsigned char *buf, unsigned short len)
{
    // Read the register
    return (i2c_read_reg(QMI8658_slave_addr, reg, buf, len) == ESP_OK) ? 1 : 0;
}



// ISR callback function (reserved)
void IRAM_ATTR DOF_INT1_isr_handler(void* arg) {
}

void IRAM_ATTR DOF_INT2_isr_handler(void* arg) {
}

void qmi8658_gpio_int_init(void)
{
    gpio_config_t io_conf = {
        .intr_type = GPIO_INTR_DISABLE,
        .mode = GPIO_MODE_INPUT,
        .pin_bit_mask = (1ULL << DOF_INT1) | (1ULL << DOF_INT2),
        .pull_up_en = GPIO_PULLUP_ENABLE,
        .pull_down_en = GPIO_PULLDOWN_DISABLE
    };
    gpio_config(&io_conf);

    // Registration interruption service
    // gpio_install_isr_service(0);
    // gpio_isr_handler_add(DOF_INT1, DOF_INT1_isr_handler, (void*)DOF_INT1);

    // gpio_install_isr_service(0);
    // gpio_isr_handler_add(DOF_INT2, DOF_INT2_isr_handler, (void*)DOF_INT2);
...
void QMI8658_read_acc_xyz(float acc_xyz[3])
{
	...
	// ESP_LOGI(TAG,"fis210x acc:	%f	%f	%f\n", acc_xyz[0], acc_xyz[1], acc_xyz[2]);
}
...
void QMI8658_doCtrl9Command(enum QMI8658_Ctrl9Command cmd)
{
	...
	qmi8658_gpio_int_init();
	
	while (1)
	{
		val = QMI8658_readStatus1();
		printf("3 :%d \r\n",val);
		printf("1 :%d \r\n",gpio_get_level(DOF_INT1));
		if (val & 0x04)
			break;
...
unsigned char QMI8658_init(void)
{
	...
	while (iCount < 2)
	{
		...
		while ((QMI8658_chip_id != 0x05) && (retry++ < 5))
		{
			...
			// ESP_LOGI(TAG,"QMI8658Register_WhoAmI = 0x%x", QMI8658_chip_id);
		}
	...
	if (QMI8658_chip_id == 0x05)
	{
		// ESP_LOGI(TAG,"QMI8658_init slave=0x%x  QMI8658Register_WhoAmI=0x%x 0x%x", QMI8658_slave_addr, QMI8658_chip_id, QMI8658_revision_id);
		...
		if (1)
		{
            ...
            // ESP_LOGI(TAG,"QMI8658Register_Ctrl1=0x%x", read_data);
            // ESP_LOGI(TAG,"QMI8658Register_Ctrl2=0x%x", read_data);
            // ESP_LOGI(TAG,"QMI8658Register_Ctrl3=0x%x", read_data);
            // ESP_LOGI(TAG,"QMI8658Register_Ctrl4=0x%x", read_data);
            // ESP_LOGI(TAG,"QMI8658Register_Ctrl5=0x%x", read_data);
            // ESP_LOGI(TAG,"QMI8658Register_Ctrl6=0x%x", read_data);
			// ESP_LOGI(TAG,"QMI8658Register_Ctrl7=0x%x", read_data);
		}
	...
	{
		ESP_LOGI(TAG,"QMI8658_init fail\n");
		QMI8658_chip_id = 0;
		return 0;
	}
